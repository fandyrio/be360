...
use Symfony\Component\CssSelector\Node\HashNode;
    ...
    class zonasiService{
        ...
        public function updatePesertaSIKEP($id_zonasi, $id_satker_selected = null){
            ...
            if($jlh_satker > 0){
                ...
                foreach($get_data as $list_peserta){
                    ...
                    $data_observee[$x]['bagian']=$list_peserta->NamaStruktur;
        ...
        public function getDataHakimZonasi($id_zonasi_satker){
            $data_hakim = [];
            $get_data_hakim = Trans_observee::join("trans_zonasi_satker as tzs", "tzs.IdZonaSatker", "=", "trans_observee.IdZonaSatker")
                                                ...
                                                ->get();
            foreach($get_data_hakim as $list_hakim){
                $data_hakim[]=[
        ...
        public function generateDataPenilaianHakim($id_zonasi, $id_zonasi_satker, $id_observee_peserta, $data_hakim){
            ...
            $get_data_majelis = Majelis_hakim::whereIn("nama_majelis", function($query) use($id_observee_peserta){
                $query->select("nama_majelis")
                        ->from("tref_majelis_hakim")
            ...
            foreach($get_data_majelis as $list_majelis){
                if(!in_array($list_majelis['IdObservee'], $id_observee_exception) && (int)$id_observee_peserta !== (int)$list_majelis['IdObservee'] && !in_array($list_majelis['IdObservee'], $id_observee_hakim)){
                    $id_observee_hakim[]=$list_majelis['IdObservee'];
                }
            }
            $jumlah_hakim_penilai = count($id_observee_hakim);
            //2,ambil hakim yang ada dimajelis
            // for($h=0;$h<$jumlah_hakim_majelis;$h++){
                
            // }
            $data_hakim_arg = $this->arrangeHakimJumlahPenilaiaan($data_hakim);
            if($jumlah_hakim_penilai === 3){
                for($x = 0;$x<$jumlah_hakim_penilai;$x++){
                    $data[]=[
                                'id_zonasi'=>$id_zonasi,
                                'id_zona_satker'=>$id_zonasi_satker,
                                'id_pegawai_peserta'=>$id_observee_peserta,
                                'id_pegawai_penilai'=>$id_observee_hakim[$x],
                                'is_self_assessment'=>false,
                                'id_jabatan_plt'=>null,
                                "index_plt"=> 0
                            ];
                    for($dh=0;$dh<count($data_hakim);$dh++){
                        if($data_hakim[$dh]['id_observee'] === $id_observee_hakim[$x]){
                            $data_hakim[$dh]['jumlah_penilaian'] += 1;
                        }
                    }
                }
                ...
                for($x = 0;$x<$jumlah_hakim_penilai;$x++){
                    ...
                    $id_observee_hakim_penilai[]=$id_observee_hakim[$x];
                }
                    ...
                    for($jh_arg = 0;$jh_arg < count($data_hakim_arg);$jh_arg++){
                        if((int)$data_hakim_arg[$jh_arg]['id_observee'] !== (int)$id_observee_peserta && !in_array($data_hakim_arg[$jh_arg]['id_observee'], $id_observee_hakim_penilai)){
                            $data[]=[
                                ...
                                'id_zona_satker'=>$id_zonasi_satker,
                                'id_pegawai_peserta'=>$id_observee_peserta,
                                'id_pegawai_penilai'=>$data_hakim_arg[$jh_arg]['id_observee'],
                                'is_self_assessment'=>false,
                                'id_jabatan_plt'=>null,
                                "index_plt"=> 0
                            ];
                            for($dh=0;$dh<count($data_hakim);$dh++){
                                if($data_hakim[$dh]['id_observee'] === $data_hakim_arg[$jh_arg]['id_observee']){
                                    $data_hakim[$dh]['jumlah_penilaian'] += 1;
                                }
                            }
                            $id_observee_hakim_penilai[]=$data_hakim_arg[$jh_arg]['id_observee'];
                            break;
                        }
                    }
                }
                ...
                for($y=0;$y<count($data_hakim_arg);$y++){
                    if($limit <= 3){
                        for($x=0;$x<$jumlah_hakim_penilai;$x++){
                            if((int)$data_hakim_arg[$y]['id_observee'] === (int)$id_observee_hakim[$x]){
                                $data[]=[
                                    'id_zonasi'=>$id_zonasi,
                                    'id_zona_satker'=>$id_zonasi_satker,
                                    'id_pegawai_peserta'=>$id_observee_peserta,
                                    'id_pegawai_penilai'=>$data_hakim_arg[$y]['id_observee'],
                                    'is_self_assessment'=>false,
                                    'id_jabatan_plt'=>null,
                                    "index_plt"=> 0
                                ];
                                for($dh=0;$dh<count($data_hakim);$dh++){
                                    if($data_hakim[$dh]['id_observee'] === $data_hakim_arg[$y]['id_observee']){
                                        $data_hakim[$dh]['jumlah_penilaian'] += 1;
                                    }
                                }
                                $limit+=1;
        ...
        public function arrangeHakimJumlahPenilaiaan($data_hakim){
            return collect($data_hakim)
                        ->sortBy("jumlah_penilaian")
                        ->values()
                        ->toArray();
        }
        ...
        public function getPeserta($id_zonasi_enc){
            ...
            foreach($get_satker_zonasi as $list_satker){
                ...
                foreach($get_jabatan_peserta as $list_jabatan_peserta){
                    ...
                    if(!is_null($list_jabatan_peserta['id_jabatan_gabungan'])){
                        ...
                        for($a=0;$a<$jlh_gabungan_pack;$a++){
                            Log::warning("Cocok di id jabatan gabungan dengan id ".$jabatan_gabungan_pack[$a]['id']." : ".$list_jabatan_peserta['id_jabatan_gabungan']);
                            Log::warning("Jumlah gabungan pack ".$jlh_gabungan_pack);
                    ...
                    foreach($getPeserta as $list_peserta){
                        ...
                        if((int)$list_peserta['id_kelompok_jabatan'] === (int)$list_jabatan_peserta['id_kelompok_jabatan'] && (int)$list_satker['IdZonaSatker'] === (int)$list_peserta['IdZonaSatker']){
                            ...
                            if($include === "true"){
                                ...
                                $data_peserta[$index_satker][$variable][${"index_{$variable}"}]['bagian']=$list_peserta['bagian'];
                ...
                if(isset($data_peserta[$index_satker]['wakil_ketua_pengadilan']) && !isset($data_peserta[$index_satker]['ketua_pengadilan'])){
                    ...
                    $data_peserta[$index_satker]["ketua_pengadilan"][0]['bagian']="Ketua Pengadilan";
                ...
                if(!isset($data_peserta[$index_satker]['juru_sita']) && (int)$list_satker['IdSatkerBanding'] !== (int)$list_satker['IdSatker']){
                    ...
                    $data_peserta[$index_satker]["juru_sita"][0]['bagian']="Juru Sita";
                    $data_peserta[$index_satker]["juru_sita"][0]['is_plt']="true";
                    $data_peserta[$index_satker]['juru_sita'][0]['index_jabatan_plt']=0;
                ...
                if((int)$list_satker['jumlah_panmud'] !== (int)$jlh_panmud){
                    ...
                    //ambil exists panmud
                    $get_exists_panmud=Trans_observee::where("id_kelompok_jabatan", 31)
                                        ->where("IdZonaSatker", $list_satker['IdZonaSatker'])
                                        ->get();
                    $exists_panmud=[];
                    foreach($get_exists_panmud as $list_panmud){
                        $exists_panmud[]=$list_panmud['bagian'];
                    }

                    $get_panmud_satker=DB::select("CALL SPGetPanmudSatker('".$list_satker['IdSatker']."')");
                    $panmud_satker_arr=[];
                    ...
                    $panmud_not_exists=[];
                    for($a=0;$a<count($panmud_satker_arr);$a++){
                        $existed=false;
                        for($b=0;$b<count($exists_panmud);$b++){
                            if($exists_panmud[$b] === $panmud_satker_arr[$a]){
                                $existed = true;
                            }
                        }
                        if($existed === false){
                            $panmud_not_exists[]=$panmud_satker_arr[$a];
                        }
                    }
            ...
            for($s=0;$s<=$jumlah_satker;$s++){
                $data_hakim = null;
                $data_hakim = $this->getDataHakimZonasi($id_zonasi_satker[$s]);
                $jumlah_hakim = count($data_hakim);
                        ...
                        if($data_peserta[$s][$variable_jabatan_peserta][$a]['is_plt'] === "false"){
                            ...
                            if($data_peserta[$s][$variable_jabatan_peserta][$a]['is_plt'] === "false"){
                                $data[]=[
                                    ...
                                    'id_zona_satker'=>$data_peserta[$s][$variable_jabatan_peserta][$a]['id_zona_satker'],
                                    'id_pegawai_peserta'=>$data_peserta[$s][$variable_jabatan_peserta][$a]['id_pegawai_observee'],
                                    'id_pegawai_penilai'=>$data_peserta[$s][$variable_jabatan_peserta][$a]['id_pegawai_observee'],
                                    // 'id_jabatan_plt'=>$get_kpt['is_plt'] === "true" ?  1 : null
                            ...
                            foreach($get_mapping as $mapping){
                                ...
                                if($jlh_penilai > 0){
                                    ...
                                    if($jlh_penilai > 2 && $variable_penilai === "hakim" && $variable_jabatan_peserta === "hakim"){
                                        $id_observee_peserta = $data_peserta[$s][$variable_jabatan_peserta][$a]['id_pegawai_observee'];
                                        $id_zonasi_satker_hakim = $data_peserta[$s][$variable_jabatan_peserta][$a]['id_zona_satker'];
                                        $generate = $this->generateDataPenilaianHakim($id_zonasi, $id_zonasi_satker_hakim, $id_observee_peserta, $data_hakim);
                                        $data_hakim = $generate['data_hakim'];
                                        $data_penilaian = $generate['data_penilaian'];
                                        for($dp=0;$dp<count($data_penilaian);$dp++){
                                            $data[]=$data_penilaian[$dp];
                                            $data_hakim_penilaian[]=$data_penilaian[$dp];
                                        }
                                    }else{
                                                ...
                                                if(!in_array($mapping_jabatan_kosong, $id_jabatan_kosong)){
                                                    if((int)$mapping['ada_plt'] === 1){
                                                        $data_kosong[]=[
                                                            ...
                                                            'index_plt'=>$data_peserta[$s][$variable_penilai][${"pointer_{$variable_penilai}"}]['index_jabatan_plt'],
                                                            'id_observee'=>null,
                                                            'created_at'=> date('Y-m-d H:i:s')
                                                        ];
                                                        $id_jabatan_kosong[]=$mapping_jabatan_kosong;
                                                    }
                                                }
                                            }else{
                                                $index_plt=0;
                                    ...
                                    if((int)$mapping['ada_plt'] === 1){
                                        if(!in_array($mapping_jabatan_kosong, $id_jabatan_kosong)){
                                            $data_kosong[]=[
                                                ...
                                                'id_jabatan_kosong'=>$mapping['id_jabatan_penilai'],
                                                'created_at'=>date('Y-m-d H:i:s'),
                                                'bagian'=>$mapping['jabatan_penilai'],
                                                'index_plt'=>0,
                                                'id_observee'=>null
                                            ];
                            ...
                            if($jlh_mapping > 0 && $a < $jlh_pegawai_perjabatan-1){
            ...
            if(is_null($get_zonasi_banding)){
                ...
                if($jumlah > 0 && $jumlah <= 2){
                    ...
                    foreach($get_kpt as $list_kpt){
                        ...
                        $id_nama_jabatan_new=(int)$list_kpt->IdNamaJabatan + (int)$list_kpt->IdPegawai;
                        $id_zona_satker_new = (int)$id_zonasi_satker + (int)$list_kpt->IdPegawai;
                        $data_observee[$x]['endpoint']=Hashids::encode($list_kpt->IdPegawai)."-".Hashids::encode($id_nama_jabatan_new)."-".Hashids::encode($id_zona_satker_new);
                        $data_observee[$x]['diinput_tgl']=date('Y-m-d H:i:s');
                        $data_observee[$x]['diinput_oleh']="system";
        ...
        public function getKelengkapanMajelis($id_zonasi){
            ...
            $get_majelis = Satker::join("trans_zonasi_satker as tzs", "tzs.IdSatker", "=", "v_satker.IdSatker")
                                    ...
                                    ->select("v_satker.IdSatker", "v_satker.NamaSatker", DB::raw("COUNT(DISTINCT nama_majelis) as jumlah"))
                                    ->where("tz.IdZona", $id_zonasi)
        ...
        public function getJabatanKosong($page, $id_zonasi){
            ...
            $get_jabatan_kosong=Trans_jabatan_kosong::join('tref_jabatan_peserta as tjp', 'tjp.id', '=', 'trans_jabatan_kosong.id_jabatan_kosong')
                                        ...
                                        ->join('v_satker as vs', 'vs.IdSatker', '=', 'tzs.IdSatker')
                                        ->select('vs.NamaSatker', 'tjp.jabatan', 'trans_jabatan_kosong.id_observee', 'trans_jabatan_kosong.status', 'trans_jabatan_kosong.bagian', 'vs.KodeSatker')
        ...
        public function generatePesertaTest($id_zonasi_satker){
            ...
            foreach($get_jabatan_peserta as $list_jabatan_peserta){
                ...
                if(isset($data_peserta[$index_satker]['wakil_ketua_pengadilan']) && !isset($data_peserta[$index_satker]['ketua_pengadilan'])){
                    ...
                    $data_peserta[$index_satker]["ketua_pengadilan"][0]['id_pegawai_observee']=$data_peserta[$index_satker]["wakil_ketua_pengadilan"][0]['id_pegawai_observee'];
                    $data_peserta[$index_satker]["ketua_pengadilan"][0]['id_kelompok_jabatan']=$data_peserta[$index_satker]["wakil_ketua_pengadilan"][0]['id_kelompok_jabatan'];
                    $data_peserta[$index_satker]["ketua_pengadilan"][0]['id_zona_satker']=1;
                ...
                if(!isset($data_peserta[$index_satker]['panitera_pengganti'])){
                    ...
                    for($x=0;$x<count($data_pp);$x++){
                        ...
                        $data_peserta[$index_satker]["panitera_pengganti"][$x]['id_kelompok_jabatan']=$data_pp[$x]['id_kelompok_jabatan'];
                        $data_peserta[$index_satker]["panitera_pengganti"][$x]['id_zona_satker']=1;
                        $data_peserta[$index_satker]["panitera_pengganti"][$x]['bagian']="Panitera Pengganti";
                        $data_peserta[$index_satker]["panitera_pengganti"][$x]['is_plt']="true";
                        $data_peserta[$index_satker]['panitera_pengganti'][$x]['index_jabatan_plt']=$data_pp[$x]['index_jabatan_plt'];
                        $data_peserta[$index_satker]["panitera_pengganti"][$x]['jlh_menilai']=0;
